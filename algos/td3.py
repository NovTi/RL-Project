...
def handle_exp_id(exp_id, env, seed=None):
    # exp_id: td3_lr05_cp40_tau05
    base_lr = 1e-5
    base_noise_clip = 0.01
    base_tau = 0.001
    ...
    target_noise_clip=0.5  # 0.40, 0.5, 0.60
    tau=0.005        # 0.001*5, 0.001*10, 0.001*15

    # learning rate
    learning_rate = float(config_lst[0][2:]) * base_lr
    # clip range
    target_noise_clip = float(config_lst[1][2:]) * base_noise_clip
    # tau
    tau = float(config_lst[2][3:]) * base_tau
...
if __name__ == '__main__':
    ...
    # Create the model
    td3_model = handle_exp_id(args.exp_id, env, seed=args.seed)

    # kidding me? 12mins?
    # Train the agent
    td3_model.learn(total_timesteps=3e6, log_interval=100, progress_bar=False)

    # Save the model
    save_model_path = os.path.join(log_dir, 'td3_model')
    td3_model.save(save_model_path)
    print(td3_model.policy)
